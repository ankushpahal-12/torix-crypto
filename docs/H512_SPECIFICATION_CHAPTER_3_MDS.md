...
$$
w_H(\mathbf{v}) = | \{ j \in \{0, 1, 2, 3\} : v_j \ne \mathtt{0x00} \} |
$$

The differential branch number $\mathcal{B}(\mathbf{M})$ of a linear transformation $\mathbf{M}$ is defined as:

$$
\mathcal{B}(\mathbf{M}) = \min_{\mathbf{v} \in (\mathbb{F}_{2^8})^4 \setminus \{\mathbf{0}\}} ( w_H(\mathbf{v}) + w_H(\mathbf{M} \mathbf{v}) )
$$
...
**Conclusion:**  
$\mathbf{M}_{\text{MDS}}$ is rigorously MDS and achieves differential branch number **$\mathcal{B} = 5$**. Q.E.D.
...
$$
\begin{aligned}
z_0 &= v_0 \oplus (v_0 \oplus v_1 \oplus v_2 \oplus v_3) \oplus ( 02 \otimes (v_0 \oplus v_1) ) \\
...
$$
\text{xtime}_{64}(X) = ( (X \ll 1) \wedge \ \mathbf{M}_{\text{mask1}} ) \oplus \left( \left( \frac{X \wedge \ \mathbf{M}_{\text{mask2}}}{2^7} \right) \times \mathtt{0x1B} \right)
$$
